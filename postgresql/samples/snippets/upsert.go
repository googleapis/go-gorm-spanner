...
// Upsert shows how to use INSERT ... ON CONFLICT DO UPDATE and INSERT ... ON CONFLICT DO NOTHING
// with Spanner PostgreSQL and gorm.
//
...
func Upsert(projectId, instanceId, databaseId string) error {
	...
	// Spanner PostgreSQL does not support updating individual columns in an insert-or-update statement.
	...
	// Execute an INSERT ... ON CONFLICT DO NOTHING statement for the singers + one additional singer.
	singers = append(singers, sample_model.Singer{
		FirstName: sql.NullString{String: "Eddie", Valid: true},
		LastName:  "Threepwood",
		Active:    false})
	// Use a OnConflict{DoNothing: true} clause to execute INSERT ... ON CONFLICT DO NOTHING
