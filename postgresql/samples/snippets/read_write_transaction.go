...
// ReadWriteTransaction shows how to use read/write transactions with Spanner PostgreSQL and gorm.
...
// an Aborted error should be retried. The Spanner gorm library provides the helper
...
func ReadWriteTransaction(projectId, instanceId, databaseId string) error {
	...
	if err := spannergorm.RunTransaction(context.Background(), db, func(tx *gorm.DB) error {
		...
		for _, venue := range venues {
			venue.VenueDetails = sample_model.NullJsonB{
