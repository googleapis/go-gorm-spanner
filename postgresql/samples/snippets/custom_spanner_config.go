...
// CustomSpannerConfig shows how to connect GORM to Spanner PostgreSQL with a custom configuration
...
func CustomSpannerConfig(projectId, instanceId, databaseId string) error {
	...
	// Create a Connector for Spanner PostgreSQL and open a sql.DB using this connector.
	// Then assign the sql.DB as the connection pool that should be used by gorm.
	c, err := spannerdriver.CreateConnector(config)
	sqlDb := sql.OpenDB(c)

	db, err := gorm.Open(
		spannerpg.New(postgres.Config{Conn: sqlDb}),
