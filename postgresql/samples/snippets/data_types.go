...
type AllNullTypes struct {
	ID           int64
	ColBool      spanner.NullBool    `gorm:"type:boolean"`
	ColDate      spanner.NullDate    `gorm:"type:date"`
	ColFloat32   spanner.NullFloat32 `gorm:"type:float4"`
	ColFloat64   spanner.NullFloat64 `gorm:"type:float8"`
	ColInt64     spanner.NullInt64   `gorm:"type:bigint"`
	ColJson      spanner.PGJsonB     `gorm:"type:jsonb"`
	ColNumeric   spanner.PGNumeric   `gorm:"type:numeric"`
	ColString    spanner.NullString  `gorm:"type:varchar"`
	ColTimestamp spanner.NullTime    `gorm:"type:timestamptz"`
}

// DataTypes shows the supported data type mappings for Spanner PostgreSQL when using gorm.
// This sample only contains samples for 'simple' data types. See the array_data_type.go file
// for a sample using arrays.
//
...
func DataTypes(projectId, instanceId, databaseId string) error {
	...
	rowAllTypes := AllTypes{
		...
		ColNumeric:   spanner.PGNumeric{Numeric: "3.14", Valid: true},
		ColString:    "test",
