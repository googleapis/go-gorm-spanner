...
type TicketSaleWithNullArray struct {
	gorm.Model
	CustomerName string
	Seats        spannergorm.NullStringArray `gorm:"type:text[]"`
...
type TicketSaleWithNativeArray struct {
	gorm.Model
	CustomerName string
	Seats        spannergorm.StringArray `gorm:"type:text[]"`
	Concert      *sample_model.Concert
	ConcertId    int64
}
...
// ArrayDataType shows how to map array columns in Spanner PostgreSQL when using gorm.
//
