...
type Singer struct {
	...
	FullName string `gorm:"->;type:varchar generated always as (CASE WHEN first_name IS NULL THEN last_name WHEN last_name  IS NULL THEN first_name ELSE first_name || ' ' || last_name END) stored;"`
	Active   bool
	Albums   []Album
}
...
type Venue struct {
	...
	Name         string
	Description  string
	VenueDetails spanner.PGJsonB `gorm:"type:jsonb"`
}
...
func RunSample(w io.Writer, connString string) error {
	db, err := gorm.Open(spannerpg.New(postgres.Config{
		DSN: connString,
	}), &gorm.Config{PrepareStmt: true, IgnoreRelationshipsWhenMigrating: true, Logger: logger.New(
...
func CreateSinger(db *gorm.DB, firstName, lastName string) (int64, error) {
	...
	// the insert statement. (This currently does not work on the Emulator, and is therefore disabled here)
...
func UpdateDataWithJsonColumn(w io.Writer, db *gorm.DB) error {
	if err := db.Save(&Venue{
		...
		VenueDetails: spanner.PGJsonB{Value: []VenueDetails{
...
func QueryWithJsonParameter(w io.Writer, db *gorm.DB) error {
	var venues []Venue
	if err := db.Find(&venues, "venue_details->>'rating' = $1->>'rating'", spanner.PGJsonB{Value: VenueDetails{
		Rating: spanner.NullFloat64{Float64: 9, Valid: true},
	}, Valid: true,
	}).Error; err != nil {
		return err
	...
	if len(venues) == 0 {
		_, _ = fmt.Fprintf(w, "Venue not found\n")
	} else {
