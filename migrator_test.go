...
func setupTestGormConnectionWithCustomConfig(t *testing.T, config func(config *spanner.ClientConfig, opts *[]option.ClientOption)) (db *gorm.DB, server *testutil.MockedSpannerInMemTestServer, teardown func()) {
	server, _, serverTeardown := setupMockedTestServer(t)
	dsn := fmt.Sprintf("%s/projects/p/instances/i/databases/d?useplaintext=true", server.Address)
	c, err := spannerdriver.CreateConnector(dsn, config)
	if err != nil {
		serverTeardown()
		t.Fatal(err)
	}
