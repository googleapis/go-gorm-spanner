...
func (m spannerMigrator) FullDataTypeOf(field *schema.Field) (expr clause.Expr) {
	...
	sequence := field.Tag.Get(gormSpannerSequenceTag)
	if field.AutoIncrement && sequence == "" {
		kind := m.DefaultSequenceKind
		if kind == "" {
			kind = DefaultSequenceKind
		}
		if kind != DisableIdentityColumns {
			if kind == AutoIncrementIdentityColumns {
				expr.SQL += " AUTO_INCREMENT"
			} else {
				expr.SQL += fmt.Sprintf(" GENERATED BY DEFAULT AS IDENTITY (%s)", kind)
			}
		}
	}
...
func (m spannerMigrator) shouldUseSequence(f *schema.Field) bool {
	sequence := f.Tag.Get(gormSpannerSequenceTag)
	return (sequence != "" || m.DefaultSequenceKind == DisableIdentityColumns) && f.AutoIncrement && f.HasDefaultValue && f.DefaultValue == "" && f.DefaultValueInterface == nil
}
